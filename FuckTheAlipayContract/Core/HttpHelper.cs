...
namespace FuckTheAlipayContract.Core
{
    ...
    public class HttpHelper
    {
        ...
        private void SetProxy()
        {
            switch (item.ProxyType)
            {
                ...
                //使用代理，根据IP、UserName、PassWord配置
                case ProxyType.Default:
                    if (!string.IsNullOrWhiteSpace(item.ProxyIp) && !string.IsNullOrEmpty(item.ProxyUserName) && !string.IsNullOrEmpty(item.ProxyPwd))
                    {
                        //带帐号密码的 
                        WebProxy myProxy = new WebProxy(item.ProxyIp);
                        myProxy.Credentials = new NetworkCredential(item.ProxyUserName, item.ProxyPwd);
                        request.Proxy = myProxy;
                    }
                    else if (!string.IsNullOrWhiteSpace(item.ProxyIp))
                    {
                        //不带帐号密码
                        WebProxy myProxy = new WebProxy(item.ProxyIp);
                        request.Proxy = myProxy;
    ...
    public class HttpItem
    {
        ...
        /// 请求的代理设置 默认为Null。
        /// Null  不使用任何代理。
        /// Default  使用代理，根据IP、UserName、PassWord配置。
        /// IEProxy  使用IE代理，同时会检测到动态配置 。
        /// </summary>
        public ProxyType ProxyType
        {
            get { return _proxyType; }
            set { value = _proxyType; }
        }
        private ProxyType _proxyType = ProxyType.Null;
        /// <summary>
        /// 代理 服务器用户名
        /// </summary>
    ...
    public enum ResultCookieType
    {
        ...
        CookieCollection,
        /// <summary>
        /// CookieContainer cookie容器 直接使用这个 简单粗暴
        /// </summary>
        CookieContainer
    }
    ...
    public enum ProxyType
    {
        ...
        Default,
        /// <summary>
        /// 使用IE的代理，同时会检测到动态配置 >>从 Internet Explorer 运行的脚本、从自动配置项或者从 DHCP 或 DNS 查找生成的任何动态设置
        /// </summary>
        IEProxy
    }
